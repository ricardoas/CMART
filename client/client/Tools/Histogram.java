...
public class Histogram {
	 ...
	 * @param num - The number of the entry to be incremented by one
	 */
	public void add(long num){
		int bin=(int)(num/binSize);
		if (bin>=hist.length)
			bin=hist.length-1;
		hist[bin]+=1;
		numEntries++;
	 ...
	 * @param percentile - n in decimal form (eg. 90th percentile = 0.9)
	 */
	public long getPercentile(double percentile){
		long valuesChecked=0;
		int bin=0;
		while(valuesChecked<(percentile*(double)numEntries)){
			valuesChecked+=hist[bin];
			bin++;
		}
