...
/**
 * This is the controller for the prefetchImageValue page
 * 
 * @author Andrew (asfox@cmu.edu)
 * @since 0.1
 * @version 1.0
 * @date 23rd Aug 2012
 * 
...
public class ChangePrefetchValueController extends PageController{
	private static final GlobalVars GV = GlobalVars.getInstance();

	// Variables passed in the request
	private long userID = -1;
	private String authToken = null;
	private int prefetchImageValue=0;
	...
	public void checkInputs(HttpServletRequest request){
		...
		if(request != null){
			...
			try{
				prefetchImageValue = CheckInputs.checkPrefetchImageValue(request);
			}
			catch(Error e){
				prefetchImageValue = 0;
				this.errors.add(e);
			}
	...
	public void processHTML4() {
		...
		if(prefetchImageValue==0){
			GlobalVars.PREFETCH_IMAGES=false;
			GlobalVars.PREFETCH_IMAGES_NUM=0;
		}
		else if(prefetchImageValue>0){
			GlobalVars.PREFETCH_IMAGES=true;
			GlobalVars.PREFETCH_IMAGES_NUM=prefetchImageValue;
		}
