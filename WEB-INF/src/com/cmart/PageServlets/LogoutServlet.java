...
public class LogoutServlet extends HttpServlet {
	...
	private static final long serialVersionUID = 3475041456628458743L;
	...
	public void makePage(HttpServletRequest request, HttpServletResponse response, Boolean isGet)  throws ServletException, IOException {
		if(request !=null && response != null && !FaultController.fault(response)){
			...
			LogoutController vars = new LogoutController();
			int loopCount = 0;
			do{
			...
			// If there are no errors log the user out
			...
			if(!vars.useHTML5()){
				...
				Header.writeHeaderNew(out, title, "logout",vars.getUserIDString(), vars.getAuthTokenString());
				...
				if(GV.COLLECT_STATS){
					...
					GV.addStats(request, out, vars, 8);
				}
	 ...
	 * @author Andy (andrewtu@cmu.edu, turner.andy@gmail.com)
	 */
	public void createHTML4(PrintWriter out, LogoutController vars, Boolean isGet){
		...
		try{
			...
			if(vars.getIsLoggedOut()){
				out.println("Congratulations, you have logged out");
			}
			else{
				out.println("You were not logged out");
			}
	 ...
	 * @author Andy (andrewtu@cmu.edu, turner.andy@gmail.com)
	 */
	public void createHTML5(PrintWriter out, LogoutController vars, Boolean isGet){
		if(out != null && vars != null)
		try{
			// Get the data needed to display the page
			vars.getHTML5Data();
