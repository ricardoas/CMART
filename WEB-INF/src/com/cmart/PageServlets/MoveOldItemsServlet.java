...
/**
 * This servlet moves expired items to the 'old' tables
 * 
 ...
 * 
 */
@WebServlet(name="MoveOldItemsServlet", urlPatterns="/moveolditems")
public class MoveOldItemsServlet extends HttpServlet {
	
	private static final long serialVersionUID = 6475041456628458743L;
	private static final String EMPTY = "";
	private static final String title = "Move old items";
	...
	public void makePage(HttpServletRequest request, HttpServletResponse response, Boolean isGet)  throws ServletException, IOException {
		if(request !=null && response != null){
			...
			MoveOldItemsController vars = new MoveOldItemsController();
			int loopCount = 0;
			do{
			...
			if(!vars.useHTML5()){
				...
				if(GV.COLLECT_STATS){
					...
					GV.addStats(request, out, vars, 9);
				}
	 ...
	 * @author Andy (andrewtu@cmu.edu, turner.andy@gmail.com)
	 */
	public void createHTML4(PrintWriter out, MoveOldItemsController vars, Boolean isGet){
		...
		try{
			...
			out.println("Items moved: " + vars.getItemsMoved());
		}
		catch(Exception e){
			e.printStackTrace();
		}
	 ...
	 * @author Andy (andrewtu@cmu.edu, turner.andy@gmail.com)
	 */
	public void createHTML5(PrintWriter out, MoveOldItemsController vars, Boolean isGet){
		if(out != null && vars != null)
		try{
			// Get the data needed to display the page
			vars.getHTML5Data();
