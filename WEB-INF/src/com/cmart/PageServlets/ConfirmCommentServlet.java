...
/**
 * This servlet confirms that a comment was entered
 * 
 ...
 * 
 */
@WebServlet(name="ConfirmCommentServlet", urlPatterns="/confirmcomment")
public class ConfirmCommentServlet extends HttpServlet {
	
	private static final long serialVersionUID = 6475041456628458743L;
	private static final String EMPTY = "";
	private static final String title = "Confirm Comment";
	...
	public void makePage(HttpServletRequest request, HttpServletResponse response, Boolean isGet)  throws ServletException, IOException {
		if(request !=null && response != null && !FaultController.fault(response)){
			...
			ConfirmCommentController vars = new ConfirmCommentController();
			int loopCount = 0;
			do{
			...
			if(!vars.useHTML5()){
				...
				if(GV.COLLECT_STATS){
					...
					GV.addStats(request, out, vars, 22);
				}
	 ...
	 * @author Andy (andrewtu@cmu.edu, turner.andy@gmail.com)
	 */
	public void createHTML4(PrintWriter out, ConfirmCommentController vars, Boolean isGet){
		...
		try{
			...
			out.println("Your comment has been successfully submitted.");
	 ...
	 * @author Andy (andrewtu@cmu.edu, turner.andy@gmail.com)
	 */
	public void createHTML5(PrintWriter out, ConfirmCommentController vars, Boolean isGet){
		if(out != null && vars != null)
		try{
			// Get the data needed to display the page
			vars.getHTML5Data();
