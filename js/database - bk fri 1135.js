var database = {
	...
	initDb: function () {
			if (window.openDatabase){
				this.db = openDatabase("CMART1101","0.02","CMART Local Database",500000);
				this.createTables();
	 ...
	 *///ANDY APPROVED
	 //TODO: only request if the rows are empty, or ts is older than X?
		...
		$.ajax({
			...
			success:function(data){
				...
				if(typeof data.categories != "undefined") {
					database.insertCategories(data.categories);
					localStorage.setItem("insertBulk","doing cats new");
				}
				else{
	 ...
	 *///ANDY APPROVED
		insertCategories: function(categories) {
			...
			this.db.transaction(function(tx) {
				var sqlInsert = "INSERT INTO categories (id,parent,name) VALUES (?,?,?)";

				for(var i = 0; i < categories.length; i = i + 1) {

					// Insert category
					tx.executeSql(sqlInsert, [categories[i].categoryID, categories[i].parent, categories[i].name], function(tx, rs) {
					}, function(tx, err) {
					});
				}
