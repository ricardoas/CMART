...
function userLogout(){
	...
	localStorage.removeItem("myAccountTS");
...
function viewItem(itemID){
	if(isEmpty(itemID) || itemID < 0){
		...
		database.getQuestionsItemPage(itemID);
		database.getCommentsItemPage(itemID);
	}
};

/*
 * Loads an item to the askquestion page
 * used by askquestion.html
 */
function questionPage(itemID){
	if(isEmpty(itemID) || itemID < 0){
		$("#itemName").html("Error: itemID not present");
	}
	else{
		database.getitemQuestionPage(itemID);
	}
};

/*
 * Loads an item to the answerquestion page
 * used by askquestion.html
 */
function answerPage(itemID,questionID){
	if(isEmpty(itemID) || itemID < 0||isEmpty(questionID)||questionID<0){
		$("#itemName").html("Error: itemID not present");
	}
	else{
		database.getDataAnswerPage(itemID,questionID);
	}
};
 ...
 * used by askquestion.html
 */
function commentPage(itemID){
	if(isEmpty(itemID) || itemID < 0){
		$("#itemName").html("Error: itemID not present");
	}
	else{
		database.getDataCommentPage(itemID);
	}
};


/*
 * Loads the bids to the Bid History Page
...
/*
 * Submits a user question for an item
 * used by askquestion.html
 */
function submitQuestion(){
	...
	var response=$.ajax({
		async: false,
		url: $.urldomain()+"/askquestion?useHTML5=1"+userID+authToken+"&itemID="+$.urlParam("itemID")+"&question="+$('#question').val(),
		type: "GET",
		dataType: "json",
		...
		success:function(data) {
			if(!isEmpty(data)) {
				if(!isEmpty(data.question)){
					database.insertQuestion(data.question,data.success);
				}
		...
		error:function(jqXHR,textStatus,errorThrown){
			alert("submitQuestion(): "+textStatus+" , "+errorThrown);
		}
	});
};

/*
 * Submits an answer to a question asked by a user
 * used by askquestion.html
 */
function submitAnswer(){
	...
	var response=$.ajax({
		async: false,
		url: $.urldomain()+"/answerquestion?useHTML5=1"+userID+authToken+"&itemID="+$.urlParam("itemID")+"&questionID="+$.urlParam("questionID")+"&answer="+$('#answer').val(),
		type: "GET",
		dataType: "json",
...
/*
 * Submits a user comment for an item
...
function submitComment(){
	...
	var response=$.ajax({
		async: false,
		url: $.urldomain()+"/commentitem?useHTML5=1"+userID+authToken+"&itemID="+$.urlParam("itemID")+"&comment="+$('#comment').val()+"&rating="+$('#rating').val(),
		type: "GET",
		dataType: "json",
		...
		success:function(data) {
			if(!isEmpty(data)) {
				if(!isEmpty(data.comment)){
					database.insertComment(data.comment,data.success);
				}
					...
					if(rs.rows.length > 0) {
						...
						for(var i = 0; i < rs.rows.length; i = i + 1) {
							...
							if(rs.rows.item(i)["isOld"] === 1) {
								...
								oldBids.append("<td><div class=\"img\"><img height=\"80\" width=\"80\" src=\"../img/").append(rs.rows.item(i).thumbnail).append("\" alt=\"").append(rs.rows.item(i).id).append("\" /></div></td>");
								...
								bids.append("<td><div class=\"img\"><img height=\"80\" width=\"80\" src=\"../img/").append(rs.rows.item(i).thumbnail).append("\" alt=\"").append(rs.rows.item(i).id).append("\" /></div></td>");
...
function sellitem(){
	...
	var categoryID=0;
	if($("#categoryID4").val()!=null&& $("#categoryID4").val()!=-1)
		categoryID=$("#categoryID4").val();
	else if($("#categoryID3").val()!=null&& $("#categoryID3").val()!=-1)
		categoryID=$("#categoryID3").val();
	else if($("#categoryID2").val()!=null&& $("#categoryID2").val()!=-1)
		categoryID=$("#categoryID2").val();
	else if($("#categoryID1").val()!=null&& $("#categoryID1").val()!=-1)
		categoryID=$("#categoryID1").val();
	else if($("#categoryID0").val()!=null&& $("#categoryID0").val()!=-1)
		categoryID=$("#categoryID0").val();

	var response=$.ajax({
		async: false,
		url:$.urldomain()+"/sellitem?useHTML5=1"+userID+authToken+"&name="+$("#sell_name").val()+"&description="+$("#sell_description").val()+"&startPrice="+$("#sell_startPrice").val()+"&reservePrice="+$("#sell_reservePrice").val()+"&buyNowPrice="+$("#sell_buyNowPrice").val()+"&categoryID="+categoryID+"&endDate="+$("#sell_endDate").val()+"&quantity="+$("#sell_quantity").val(),
		type: "GET",
		dataType: "json",
		success:function(data){
