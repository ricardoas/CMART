...
public class GlobalVars implements ServletContextListener{
	...
	private static final String configFile  = "/cmart/cmartconfig.txt";
	...
	private GlobalVars(boolean nothing){
		...
		try{
			...
			try{
				...
				System.err.println("C-MART is looking for it's config file at " + configFile);
				System.err.println("The location of the config file can be changed in com.cmart.Data.GlobalVars if required");
			}
				...
				if(line!=null){
					if(!line.startsWith(commentLine)){
						...
						if(values.length >= 2){
							...
							if(key!=null && key.startsWith("IP")){
								if(key.equals("IPSQL")){
									MYSQL_IPS.add(valueBuffer.toString());
								}
								else if(key.equals("IPCASS")){
									CASSANDRA_IPS.add(valueBuffer.toString());
								}
								else if(key.equals("IPSOLR")){
									SOLR_IPS.add(valueBuffer.toString());
								}
								else{
			...
			// MONGODB IP
			value = config.get("gfs_ip");
			if (value != null) {
				GFS_IP = value;
			} else {
				valueBuffer.append("warning: 'gfs_ip' was set incorrectly..\n");
			}
			value = config.get("gfs_enabled");
			if(value != null){
				if(value.equals("1") || value.toLowerCase().equals("true")) GFS_ENABLED = Boolean.TRUE;
				else if(value.equals("0") || value.toLowerCase().equals("false")) GFS_ENABLED = Boolean.FALSE;
			}
			if(value==null || GFS_ENABLED==null){
				GFS_ENABLED = Boolean.FALSE;
				valueBuffer.append("Warning: 'gfs_enabled' was not set correctly. Defaulting to false.\n");
			}
			...
			value = config.get("first_is_master");
			if(value != null){
				if(value.equals("1") || value.toLowerCase().equals("true")) firstIsMaster = Boolean.TRUE;
				else if(value.equals("0") || value.toLowerCase().equals("false")) firstIsMaster = Boolean.FALSE;
			}
			...
			System.out.println("Could not open config file '/cmart/cmartconfig.txt' in " + System.getProperty("user.dir"));
			e.printStackTrace();
		}
	}
	...
	// MONGO Database information for GFS
	public static String GFS_IP;
	public static Boolean GFS_ENABLED;
	...
	public static ArrayList<String> MYSQL_IPS = new ArrayList<String>();
	public static Boolean firstIsMaster = false;
	
	// CASSANDRA Database information
	public static String CASSANDRA_DATABASE_URL;
	public static String CASSANDRA_DATABASE_DRIVER;
	public static ArrayList<String> CASSANDRA_IPS = new ArrayList<String>();

	// SOLR Information
	public static Boolean SOLR_ENABLED;
	public static String SOLR_URL;
	public static int SOLR_MAX_CONNS_PER_HOST;
	public static ArrayList<String> SOLR_IPS = new ArrayList<String>();
