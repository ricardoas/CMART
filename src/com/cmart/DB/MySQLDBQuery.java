...
public class MySQLDBQuery extends DBQuery{
	protected int maxSQLConnectionsCache = 200;
	private static MySQLDBQuery single = null;
	private static int CONNECTION_POOL = GlobalVars.MY_CONN_CACHE;

	private DataSource ds;

	public MySQLDBQuery(String forT2testingOnly){
		MySQLDBQuery.getInstance();
	}
	
	private MySQLDBQuery(){
		super();
		init();
	}

	public static MySQLDBQuery getInstance(){
		if (single == null) {
			synchronized (MySQLDBQuery.class) {
				if (single == null) {
					single = new MySQLDBQuery();
				}
			}
		}
	...
	public double insertBidDB(long userID, long itemID, int quantity, double bid, double maxBid){	
		...
		do {
			...
			if (conn != null) {
				PreparedStatement lockTableStatement = null;
				...
				PreparedStatement unlockTableStatement = null;
				try {
					/*
					...
					lockTableStatement = conn.prepareStatement("LOCK TABLES bids WRITE, items WRITE");
					...
					// Unlock the tables
					unlockTableStatement = conn.prepareStatement("UNLOCK TABLES");
					...
					this.closeSmt(lockTableStatement);
					this.closeSmt(insertBidStatement);
					this.closeSmt(getMaxBidStatement);
					this.closeSmt(updateItemStatement);
					this.closeSmt(unlockTableStatement);

					// Just make sure we have unlocked the tables
					try{
						unlockTableStatement = conn.prepareStatement("UNLOCK TABLES");
						unlockTableStatement.executeUpdate();
					}
		...
		do {
			...
			if (conn != null) {
				...
				try {
					...
					lockTableStatement = conn.prepareStatement("LOCK TABLES payments WRITE, purchased WRITE, items WRITE");
			...
			if (conn != null) {
				...
				try {
					...
					if(emailSplit.length==2){
						statement = conn.prepareStatement("SELECT `email` FROM `users` WHERE MATCH (email) AGAINST ('+\"" + emailSplit[0] +" "+emailSplit[1]+"\"' IN BOOLEAN MODE);");
					}
					else{
	...
	public long moveEndedItemsDB() throws Exception{
		...
		//TODO: does not create purcahses!!
	...
	public Question getQuestion(long questionID) throws Exception{
		...
		if(conn != null){
			try{
				...
				System.err.println("MySQLQuery (getQuesion): Could not get the question");
				e.printStackTrace();
				throw e;
			}
