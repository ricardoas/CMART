...
public abstract class DBQuery {
	...
	ArrayList<CommonsHttpSolrServer> SOLR_SERVERS = new ArrayList<CommonsHttpSolrServer>();
	static int currentSolr = 0;
	static int countSolr = 0;
	...
	public void initSolr(){
		if(GlobalVars.SOLR_ENABLED)
			try {
				if(GlobalVars.SOLR_IPS==null || GlobalVars.SOLR_IPS.size()==0){	
					CommonsHttpSolrServer srv = new CommonsHttpSolrServer(GlobalVars.SOLR_URL);
					srv.setDefaultMaxConnectionsPerHost(GlobalVars.SOLR_MAX_CONNS_PER_HOST);
					
					srv.setSoTimeout(10000);
					srv.setConnectionTimeout(1000);
					srv.setMaxTotalConnections(500);
					srv.setFollowRedirects(false);
					srv.setParser(new XMLResponseParser());
					
					SOLR_SERVERS.add(srv);
				}
				else{
					// Parse the URL so we can insert the IP
					String urlFront = null;
					String urlBack = null;
					urlFront = GlobalVars.SOLR_URL.split("/")[0] + "//";
					urlBack = ":"+GlobalVars.SOLR_URL.split("/")[2].split(":")[1]+ "/" + GlobalVars.SOLR_URL.split("/")[3];
					countSolr = GlobalVars.SOLR_IPS.size();
					currentSolr=0;
					
					// Make the connections
					for(int i=0; i<countSolr; i++){
						String url = urlFront + GlobalVars.SOLR_IPS.get(i)+ urlBack;
						
						CommonsHttpSolrServer srv = new CommonsHttpSolrServer(url);
						srv.setDefaultMaxConnectionsPerHost(GlobalVars.SOLR_MAX_CONNS_PER_HOST);
	...
	public synchronized CommonsHttpSolrServer getSolrServer(){
		if(currentSolr >= countSolr) currentSolr=0;
		CommonsHttpSolrServer ret = SOLR_SERVERS.get(currentSolr);
		currentSolr++;
	...
	public void forceCommitSolr(){
		...
		if(!GlobalVars.SOLR_ENABLED && solrServer!=null)
			try {
				solrServer.commit();
			} catch (Exception e) {
				e.printStackTrace();
	...
	public boolean addToSolr(long id, String name, String description, double currentBid, Date endDate){
		...
		try {
			solrServer.add(doc);
			if(rand.nextDouble()<solrCommit) solrServer.commit();
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		}
	...
	* TODO: social stuff
	*/
	public abstract boolean insertFriendRequest(long fromID, String toName, String message);
	
	public abstract ArrayList<Message> getMessages(long toID);
	
	public abstract String getFromName(long fromID);
	
	public abstract ArrayList<WallPost> getWallPosts(long toID);
	
	public abstract ArrayList<WallPost> getReplies(long replyID);

	public abstract ArrayList<WallPost> getNewsFeeds(long userID);

	public abstract void confirmFriendRequest(long acceptID);

	public abstract void rejectFriendRequest(long rejectID);
	
	public abstract boolean insertMessage(long fromID, long toID, String text);

	public abstract long getUserID(String user);

	public abstract ArrayList<FriendRequest> getFriendRequests(long userID);

	public abstract boolean insertWallPost(long fromID, long toID, String text);

	public abstract boolean insertReply(long formID, long replyID, String post);

	public abstract boolean hasRequestID(long id);

	public abstract boolean hasWallpostsToID(long toID);

	public abstract boolean hasWallpostsReplyID(long replyID);

	public abstract boolean areFriends(long userID, long toID);
