...
public class SellItemImagesController extends PageController{
	...
	public void saveImages(String baseURL){
		...
		if(GV.GFS_ENABLED){
			try{
				mongo = new Mongo(GV.GFS_IP, 27017);
				db = mongo.getDB("imagedb");
				gfsPhoto = new GridFS(db, "photo");
			}
			catch(Exception e){
				System.out.println("Error! Could not connect to GFS server in SellItemImagesController!!");
				...
				try {
					...
					if(GV.GFS_ENABLED){
						try{
							...
							GridFSInputFile gfsFile = gfsPhoto.createFile(file2);
							gfsFile.setFilename(filename);
							gfsFile.save();
						}
						catch(Exception e){
							System.out.println("Error! Could not write file to GFS in SellItemImagesController!");
							System.out.println(e.getMessage());
						}
					}
					...
					if(thumbnail){
						...
						*/
						...
						// Maybe write to GFS - can delete thumbnail if we use GFS
						if(GV.GFS_ENABLED){
							try{
								gfsPhoto.remove(thumbName);
								GridFSInputFile gfsFile = gfsPhoto.createFile(thumbFile);
								gfsFile.setFilename(thumbName);
								gfsFile.save();
								try{
									thumbFile.delete();
								}catch(Exception e){}
								...
								System.out.println("Error! Could not write file thumbnail to GFS in SellItemImagesController!");
								System.out.println(e.getMessage());
							}
						}
					}
					
					// IF GFS we don't need the files
					if(GV.GFS_ENABLED){
						try{
							file2.delete();
						}catch(Exception e){}
