...
/**
 * This controller is for the view user page
 * 
...
public class ViewUserController extends PageController{
	...
	public void checkInputs(HttpServletRequest request){
		...
		if(request != null){
			...
			// Get the viewUserID as we need to read a user's info
			try{
				this.viewUserID = CheckInputs.checkViewUserID(request);
			}
			catch(Error e){
	...
	public void getHTML4Data() {	
		...
		if(this.viewUserID>0)
			this.user = GV.DB.getPublicUser(this.viewUserID);
