...
public class GetNewsFeedController extends PageController {
	...
	private ArrayList<WallPost> newsfeeds = null;
	...
	public void getHTML4Data() {
		...
		newsfeeds = GV.DB.getNewsFeeds(this.userID);
	 ...
	 * @author Chaohua (chaohuam@andrew.cmu.edu, ma.chaohua@gmail.com)
	 */
	public ArrayList<WallPost> getNewsFeeds() {
		return newsfeeds;	
	}
	...
	public boolean insertWallPost() {
		if (this.errors.size() == 0)
			if (GV.DB.checkAuthToken(this.userID, this.authToken)) {
				if (this.errors.size() == 0)
					return GV.DB.insertWallPost((long) userID, (long) userID, post);
