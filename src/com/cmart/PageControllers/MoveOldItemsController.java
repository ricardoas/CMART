...
/**
 * This controller moves the old items and bids from the current tables to the 'old' tables, i.e. moves auctions that have ended
 * 
...
public class MoveOldItemsController extends PageController{
	...
	public void checkInputs(HttpServletRequest request){
		...
		//TODO: this should be limited to an admin user. But since we don't really need securty we could just let everyone
		// access it?
	...
	public void getHTML4Data() {	
		...
		try {
			this.itemsMoved = GV.DB.moveEndedItems();
		} catch (Exception e) {
			errors.add(new Error("MoveOldItemsControl: getHTML4Data could not move the items in the database",e));
		}
	...
	public void getHTML5Data(){
		...
		try {
			...
			errors.add(new Error("MoveOldItemsControl: getHTML5Data could not move the items in the database",e));
		}
