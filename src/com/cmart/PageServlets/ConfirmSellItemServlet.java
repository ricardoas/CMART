...
/**
 * This servlet confirms that the user's bid was entered
 * 
 ...
 * 
 */
@WebServlet(name="ConfirmSellItemServlet", urlPatterns="/confirmsellitem")
public class ConfirmSellItemServlet extends HttpServlet {

	private static final long serialVersionUID = 4525041555628458743L;
	private static final String EMPTY = "";
	private static final String title = "Sell Item Confirmed";
	...
	public void makePage(HttpServletRequest request, HttpServletResponse response, Boolean isGet)  throws ServletException, IOException {
		if(request !=null && response != null && !FaultController.fault(response)){
			...
			ConfirmSellItemController vars = new ConfirmSellItemController();
			int loopCount = 0;
			do{
			...
			if(!vars.useHTML5()){
				...
				if(GV.COLLECT_STATS){
					...
					GV.addStats(request, out, vars, 4);
				}
	 ...
	 * @author Andy (andrewtu@cmu.edu)
	 */
	public void createHTML4(PrintWriter out, ConfirmSellItemController vars, Boolean isGet){
		if(out != null && vars != null)
			try{
				...
				out.println("Thank you for placing an item in the auction!<BR />");
				out.println("Your item " + vars.getItemName() + " is now listed for sale.<BR />");
				out.println("Please check your user account later to see if your item sold.<BR />");
	 ...
	 * @author Andy (andrewtu@cmu.edu)
	 */
	public void createHTML5(PrintWriter out, ConfirmSellItemController vars, Boolean isGet){
		if(out != null && vars != null)
			try{
				// Get the data needed to display the page
				vars.getHTML5Data();
