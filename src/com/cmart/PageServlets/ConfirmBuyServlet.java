...
/**
 * This servlet confirms that the user bought an item
 * 
 ...
 * 
 */
@WebServlet(name="ConfirmBuyServlet", urlPatterns="/confirmbuy")
public class ConfirmBuyServlet extends HttpServlet {

	private static final long serialVersionUID = 4525086456628458743L;
	private static final String EMPTY = "";
	private static final String title = "Buy Confirmed";
	...
	public void makePage(HttpServletRequest request, HttpServletResponse response, Boolean isGet)  throws ServletException, IOException {
		if(request !=null && response != null && !FaultController.fault(response)){
			...
			ConfirmBuyController vars = new ConfirmBuyController();
			int loopCount = 0;
			do{
			...
			if(!vars.useHTML5()){
				...
				if(GV.COLLECT_STATS){
					...
					GV.addStats(request, out, vars, 3);
				}
	 ...
	 * @author Andy (andrewtu@cmu.edu)
	 */
	public void createHTML4(PrintWriter out, ConfirmBuyController vars, Boolean isGet){
		if(out != null && vars != null)
			try{
				...
				out.println("Congratulations, you have bought the item<BR />");
	 ...
	 * @author Andy (andrewtu@cmu.edu)
	 */
	public void createHTML5(PrintWriter out, ConfirmBuyController vars, Boolean isGet){
		if(out != null && vars != null)
			try{
				// Get the data needed to display the page
				vars.getHTML5Data();
