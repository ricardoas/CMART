...
import com.cmart.PageControllers.ReadMessageController;
import com.cmart.util.Message;
import com.cmart.util.StopWatch;

/**
 * This page allows the user to read message
 * 
...
public class ReadMessageServlet extends HttpServlet {
	...
	public void makePage(HttpServletRequest request,
		...
		if (request != null && response != null) {
			...
			// process the data
			ReadMessageController vars = new ReadMessageController();
			int loopCount = 0;
			do {
			...
			if (!vars.useHTML5()) {
				...
				Header.writeHeaderNew(out, title, "message",
	 ...
	 * @author Chaohua (chaohuam@andrew.cmu.edu, ma.chaohua@gmail.com)
	 */
	public void createHTML4(PrintWriter out, ReadMessageController vars,
			Boolean isGet) {
		if (out != null && vars != null)
			try {
				...
				/*
				 * Print out all of the messages for the user
				 */
				out.write("<div class=\"nine columns\">");
				ArrayList<Message> messages = vars.getMessages();
				out.println("<table>");
				out.println("<tbody>");
				...
				if (messages != null && size > 0) {
					for (int i = 0; i < size; i++) {
						...
						out.println("</td>");
						out.println("<td>");
						String fromName = vars.getFromName(message.getFromID());
					...
					out.println("<td>");
					out.println("<BR /> Sorry, there is not message to read");
					out.println("</td>");
					out.println("</tr>");
				}
				out.println("</tbody>");
	 ...
	 * @author Chaohua (chaohuam@andrew.cmu.edu, ma.chaohua@gmail.com)
	 */
	public void createHTML5(PrintWriter out, ReadMessageController vars,
			Boolean isGet) {
		if (out != null && vars != null)
			try {
				// Get the data needed to display the page
