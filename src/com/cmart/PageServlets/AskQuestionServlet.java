...
/**
 * This servlet allows user to ask questions about an item
 * 
 ...
 * 
 */
@WebServlet(name="AskQuestionServlet", urlPatterns="/askquestion")
public class AskQuestionServlet extends HttpServlet{

	private static final long serialVersionUID = 6475041456628458743L;
	private static final String EMPTY = "";
	private static final String title = "Ask Question";
	...
	public void makePage(HttpServletRequest request, HttpServletResponse response, Boolean isGet)  throws ServletException, IOException {
		if(request !=null && response != null && !FaultController.fault(response)){
			...
			AskQuestionController vars = new AskQuestionController();
			int loopCount = 0;
			do{
			...
			if(vars.getErrors().size() == 0){
				...
				vars.getHTML4Data();
					...
					vars.getHTML5Data();
				if(question = vars.submitQuestion()){
					if(timer != null)
						timer.stop();
					if(!vars.useHTML5()){
			...
			if(!vars.useHTML5()  && !redirect){
				...
				if(GV.COLLECT_STATS){
					...
					GV.addStats(request, out, vars, 20);
				}
	 ...
	 * @author Andy (andrewtu@cmu.edu) Bo (bol1@andrew.cmu.edu)
	 */
	public void createHTML4(PrintWriter out, AskQuestionController vars, Boolean isGet){
		if(out != null && vars != null)
			try{
				...
				if(item != null){
					...
					out.println("<tr><td>");
					out.println();
					out.println("<form name=\"question\" action=\"askquestion\" method=\"POST\">");
					...
					out.println("Please input your question: <br /><textarea name=\"question\" cols=\"30\" rows=\"5\" value=\"\"></textarea></tr></td>");
	 ...
	 * @author Andy (andrewtu@cmu.edu) Bo (bol1@andrew.cmu.edu)
	 */
	public void createHTML5(PrintWriter out, AskQuestionController vars, Boolean isGet){
		if(out != null && vars != null)
			try{
				// Get the data needed to display the page
				vars.getHTML5Data();
				vars.processHTML5();
				out.append("\"pageType\":\"askquestion\",");

				if(vars.getErrors().size()==0){
					out.append("\"success\":true");
					out.append(",\"question\":").append(vars.getQuestionDB().toJSON());
				}else{
