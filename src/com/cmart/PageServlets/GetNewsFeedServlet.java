...
import com.cmart.PageControllers.GetNewsFeedController;
...
public class GetNewsFeedServlet extends HttpServlet {
	...
	public void makePage(HttpServletRequest request,
		...
		if (request != null && response != null) {
			...
			// process the data
			GetNewsFeedController vars = new GetNewsFeedController();
			int loopCount = 0;
			do {
			...
			if (!vars.useHTML5()) {
				...
				Header.writeHeaderNew(out, title, "News Feed",
	 ...
	 * @author Chaohua (chaohuam@andrew.cmu.edu, ma.chaohua@gmail.com)
	 */
	public void createHTML4(PrintWriter out, GetNewsFeedController vars,
			Boolean isGet) {
		if (out != null && vars != null)
			try {
				...
				out.println("<form name=\"post\" action=\"getnewsfeed\" class=\"nice\" method=\"POST\">");
				out.println("<table>");
				...
				out.println("<tr><td><div id=\"floatright\">Update Status: </div></td>");
				...
				ArrayList<WallPost> wallPosts = vars.getNewsFeeds();
				out.println("<table>");
				out.println("<tbody>");
				...
				if (wallPosts != null && size > 0) {
					for (int i = 0; i < size; i++) {
						...
						out.println("<tr>");
						out.println("<td>");
						out.println("<form name=\"reply\" action=\"getnewsfeed\" class=\"nice\" method=\"POST\">");
						out.println("<table>");
						out.println("<div id=\"center\">");
						out.println("<input type=\"hidden\" name=\"userID\" value=\""
								+ vars.getUserIDString() + "\">");
					...
					out.println("<td>");
					out.println("<BR /> Sorry, there is no news feed to read");
					out.println("</td>");
					out.println("</tr>");
				}
				out.println("</tbody>");
	 ...
	 * @author Chaohua (chaohuam@andrew.cmu.edu, ma.chaohua@gmail.com)
	 */
	public void createHTML5(PrintWriter out, GetNewsFeedController vars,
			Boolean isGet) {
		if (out != null && vars != null)
			try {
				// Get the data needed to display the page
