...
public class ConfirmBidServlet extends HttpServlet {
	...
	public void makePage(HttpServletRequest request, HttpServletResponse response, Boolean isGet)  throws ServletException, IOException {
		if(request !=null && response != null && !FaultController.fault(response)){
			...
			ConfirmBidController vars = new ConfirmBidController();
			int loopCount = 0;
			do{
	 ...
	 * @author Andy (andrewtu@cmu.edu)
	 */
	public void createHTML4(PrintWriter out, ConfirmBidController vars, Boolean isGet){
		if(out != null && vars != null)
			try{
				...
				out.println("Thank you for placing a bid!<BR />");
				out.println("You bid " + GlobalVars.currency.format(Double.parseDouble(vars.getBid())) + " for " + vars.getItemName() + ".<BR />");
				out.println("Please check your user account later to see if you have won the auction.<BR />");
	 ...
	 * @author Andy (andrewtu@cmu.edu)
	 */
	public void createHTML5(PrintWriter out, ConfirmBidController vars, Boolean isGet){
		if(out != null && vars != null)
			try{
				// Get the data needed to display the page
				vars.getHTML5Data();
