...
import com.cmart.PageControllers.ConfirmMessageController;
...
public class ConfirmMessageServlet extends HttpServlet {
	...
	public void makePage(HttpServletRequest request,
		...
		if (request != null && response != null) {
			...
			// process the data
			ConfirmMessageController vars = new ConfirmMessageController();
			int loopCount = 0;
			do {
			...
			if (!vars.useHTML5()) {
				...
				Header.writeHeaderNew(out, title, "Message Confirmed",
	...
	public void createHTML4(PrintWriter out,
			ConfirmMessageController vars, Boolean isGet) {
		if (out != null && vars != null) {
			try {
				...
				out.println("Thank you for sending a message!<br />");
				out.println("</div>");
				out.println("</div>");
				out.println("</div>");
			} catch (Exception e) {
