...
import com.cmart.PageControllers.SendMessageController;
...
public class SendMessageServlet extends HttpServlet {
	...
	public void makePage(HttpServletRequest request,
		...
		if (request != null && response != null) {
			...
			// process the data
			SendMessageController vars = new SendMessageController();
			int loopCount = 0;
			do {
				vars.checkInputs(request);
				loopCount++;
			} while (loopCount <= vars.getProcessingLoop());
			boolean flag = vars.insertMessage();
			...
			if (!vars.useHTML5() && !redirect) {
				...
				Header.writeHeaderNew(out, title, "Send Message",
	 ...
	 * @author Chaohua (chaohuam@andrew.cmu.edu, ma.chaohua@gmail.com)
	 */
	public void createHTML4(PrintWriter out, SendMessageController vars,
			Boolean isGet) {
		if (out != null && vars != null)
			try {
				...
				out.println("<form name=\"sendmessage\" action=\"sendmessage\" class=\"nice\" method=\"POST\">");
				out.println("<table>");
				...
				out.println("<td><div id=\"floatcenter\"> <input type=\"text\" name=\"username\" size=\"50\" \"/> </div></td></tr>");
				out.println("<tr><td><div id=\"floatright\">Message: </div></td>");
	 ...
	 * @author Chaohua (chaohuam@andrew.cmu.edu, ma.chaohua@gmail.com)
	 */
	public void createHTML5(PrintWriter out, SendMessageController vars,
			Boolean isGet) {
