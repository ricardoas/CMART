...
/**
 * This servlet gets use details for the client generator
 * 
...
public class GetUsersServlet extends HttpServlet {
	...
	private static final String title = "Get Users Page";
	...
	public void makePage(HttpServletRequest request, HttpServletResponse response, Boolean isGet)  throws ServletException, IOException {
		if(request !=null && response != null){
			...
			GetUsersController vars = new GetUsersController();
	 ...
	 * @author Andy (andrewtu@cmu.edu)
	 */
	public void createHTML4(PrintWriter out, GetUsersController vars, Boolean isGet){
		if(out != null && vars != null)
			try{
				...
				long totalUsers=vars.getTotalUsers();
				if(totalUsers==0){
					ArrayList<User> users = vars.getUsers();
					ArrayList<Address> addresses = vars.getAddresses();

					if(users!=null){
						for(int i=0; i<users.size(); i++){
							out.print(users.get(i).toCSVString());
							if(addresses.get(i)!=null)
								out.println(addresses.get(i).toCSVString());
						}
					}
				}
				else{
	 ...
	 * @author Andy (andrewtu@cmu.edu)
	 */
	public void createHTML5(PrintWriter out, GetUsersController vars, Boolean isGet){
		if(out != null && vars != null)
			try{
				// Get the data needed to display the page
				vars.getHTML5Data();
